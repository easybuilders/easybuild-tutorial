...
We will use `GCC/10.2.0` as toolchain, since we know it is already installed in `/easybuild` in the prepared environment, so we define the `toolchain` easyconfig parameter:
