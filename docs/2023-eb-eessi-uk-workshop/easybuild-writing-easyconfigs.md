...
Before we dive into writing [easyconfig files](easybuild-terminology.md#easyconfig-files),
let's take a brief look at how they relate to [easyblocks](easybuild-terminology.md#easyblocks).

As we discussed [earlier](easybuild-terminology.md#easyconfig-files), an easyconfig file (`*.eb`) is required
...
* 'Critical' values for easyconfig parameters required to make installation succeed;  
    * Example: the [easyblock for Bowtie2](https://github.com/easybuilders/easybuild-easyblocks/blob/develop/easybuild/easyblocks/b/bowtie2.py)
...
* Toolchain-specific aspects of the build and installation procedure (e.g., configure options);
    * Example: the [easyblock for CP2K](https://github.com/easybuilders/easybuild-easyblocks/blob/develop/easybuild/easyblocks/c/cp2k.py) 
      will add several compiler options when compiling with gcc and gfortran, including
...
* Custom (configure) options for dependencies;  
    * Example: the [easyblock for VMD](https://github.com/easybuilders/easybuild-easyblocks/blob/develop/easybuild/easyblocks/v/vmd.py)
      will add configure options depending on the dependency list of the package.
* Interactive commands that need to be run;  
    * Example: The [easyblock for Maple](https://github.com/easybuilders/easybuild-easyblocks/blob/develop/easybuild/easyblocks/m/maple.py) 
      responds to a number of interactive questions.
* Having to create or adjust specific (configuration) files;  
    * Example: the [easyblock for Siesta](https://github.com/easybuilders/easybuild-easyblocks/blob/develop/easybuild/easyblocks/s/siesta.py)
      edits one of the configuration files through regular expressions.
* 'Hackish' usage of a generic easyblock;
    * Example: you could get creative with the use of `#` in some easyconfig parameters to effectively comment
      out some actions that a generic easyblock would do, which is *not* recommended.
* Complex or very non-standard installation procedure;  
    * Example: the [easyblock to install GCC from source](https://github.com/easybuilders/easybuild-easyblocks/blob/develop/easybuild/easyblocks/g/gcc.py),
...
the [EasyBuild documentation](https://docs.easybuild.io/implementing-easyblocks),
...
name needs to adhere to a specific convention as we [discussed earlier](easybuild-basic-usage.md#easyconfig-filenames)).
...
EasyBuild also requires that the [compiler toolchain](easybuild-terminology.md#toolchains) is specified, via the `toolchain`
easyconfig parameter.

This can either be the [`system` toolchain](easybuild-terminology.md#system-toolchain), for which a constant named `SYSTEM` is available:
...
software-specific easyblock class named `EB_example` (in a Python module named `example.py`). Software-specific
...
* `PythonPackage`: implements the installation procedure for a single Python package, by default using
   "`python setup.py install`" but other methods like using "`pip install`" are also supported (which is actually
   recommended);
...
A full overview of the available generic easyblocks is available in the
[EasyBuild documentation](https://docs.easybuild.io/version-specific/generic-easyblocks/). You can also consult the
output of `eb --list-easyblocks`, which gives an overview of *all* known easyblocks, and how they relate to each other.
...
Even though Python 2 is officially dead and buried, some scientific software still requires it,
and mixing modules where some use Python 2 and other use Python 3 doesn't work well.
...
* Names of source files and patches should not include hardcoded software versions, they usually use a
  ...
  line via the `--avail-easyconfig-templates` option, or in the [EasyBuild documentation](https://docs.easybuild.io/version-specific/easyconfig-templates).
* Source files can also be specified in ways other than just using a filename, see the [EasyBuild documentation](https://docs.easybuild.io/writing-easyconfig-files/#common_easyconfig_param_sources_alt) for more information.
* Specified checksums are usually SHA256 checksum values, but [other types are also supported](https://docs.easybuild.io/writing-easyconfig-files/#common_easyconfig_param_sources_checksums).


#### Dependencies

You will often need to list one or more [dependencies](easybuild-terminology.md#dependencies) that are required
...
See also the [EasyBuild documentation](https://docs.easybuild.io/writing-easyconfig-files/#dependency_specs) for additional options on specifying dependencies.
...
or to inject additional commands before or after them.
...
* `configopts`: string value that is added *after* the configure command, which can be used to specify
  configuration options, or to run additional commands after the configure command;
...
([remember what we saw when installing software earlier](easybuild-installing-software.md#using-installed-software)).
...
[subtoolchains](easybuild-terminology.md#toolchains) compatible with the specified toolchain to locate module for the dependencies.
...
Let us now retry, but using `--module-only` rather than redoing the whole installation.
This way, EasyBuild will still sanity check the installation, so if it creates
a module, we know the installation will work as expected.
...
[*next: Module naming schemes*](easybuild-module-naming-schemes.md) - [*(back to overview page)*](index.md)
