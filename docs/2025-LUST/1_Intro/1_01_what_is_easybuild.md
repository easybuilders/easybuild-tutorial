...
the container to install platform-optimised software in the container. 
But we have never tested this on LUMI. Instead, we do have EasyBuild recipes that 
extend existing containers with additional components and create modules to make
using those containers easier (e.g., take care of the bindings). Examples are the
containers for [PyTorch](https://lumi-supercomputer.github.io/LUMI-EasyBuild-docs/p/PyTorch/)
or the [Cray Programming Environment](https://lumi-supercomputer.github.io/LUMI-EasyBuild-docs/c/ccpe/).
...
as it is performed.
EasyBuild 5, released in early 2025, also offers improvements to the error messages that are printed
on the screen and information in the log files.
...
cannot easily overwrite those. In some cases we remove these via patches or edits to
enable cross-compiling on LUMI.
...
some MPI versions also produce slow or incorrect results on AMD CPUs (and Intel MPI on Slingshot 11 is also not free from issues depending on the version). 
...
In particular, porting from the `foss` toolchain and its subsets to the Cray `PrgEnv-gnu` environment
...
As we may not be able to fully maintain the EasyBuild structure of the repositories
on LUMI in the future, that support may not function well with the LUMI repositories.
...
EasyBuild is implemented in [**Python**](https://www.python.org), and is compatible with Python 3.6+.
Python 3.9 or more recent is highly recommended though, but on LUMI we still use the system Python,
as EasyBuild actually assumes that no specific modules are needed to get access to Python.
...
see [pypi.org/project/easybuild](https://pypi.org/project/easybuild/).
...
which is available at [**docs.easybuild.io**](https://docs.easybuild.io).

It is not unlikely that an answer to your question is available there already.
The [search box](https://docs.easybuild.io/en/latest/search.html) on the top may be helpful.
...
Also here you can use the search box on the top, to see if somebody else
...
More recently this has decreased a lot, mostly due
...
[easybuild-slack.herokuapp.com](https://easybuild-slack.herokuapp.com/).**
