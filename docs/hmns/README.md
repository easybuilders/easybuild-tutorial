...
<div align="center"><img src="img/hmns.png" width="400px"/></div>
...
consists of the `GCC/9.3.0` compiler module and the `OpenMPI/4.0.3` MPI module. Software installing using
...
Lmod has a solution for this though: it provides a separate "`module spider`" command to search
for module files throughout the entire module hierarchy. So as long as the end users are aware of this
additional command, it should not be difficult to discover which software installations exist and how
they can be accessed. The "`module spider`" command will inform the user which of the gateway modules
...
Some module files will have the same name in both module trees (like `GCC/9.3.0` for example),
...
This should take about 2 minutes in total, for generating 37 modules.
...
toolchains that use GCC 9.3.0 as a base compiler (the details of that are out of scope here).
...
If you now check the output of "`module avail`" again, you should see the `HDF5/1.10.6` module:
...
To use HDF5, we just need to load this module. We can verify that the installation works
...
* installing the missing modules for the `SciPy-bundle-2020.03-foss-2020a-Python-3.8.2.eb` in the module hierarchy we
  generated for HDF5;
...
??? success "(click to show solution)"
    ...
    * Step 0: check which modules are still missing, using `--missing` or `-M`:
      ```shell
      eb SciPy-bundle-2020.03-foss-2020a-Python-3.8.2.eb -M
      ```
    ...
    * Install the missing modules in the module hierarchy we have generated in `$HOME/hmns/modules`:
      ```shell
      eb SciPy-bundle-2020.03-foss-2020a-Python-3.8.2.eb --robot --module-only
      ```
